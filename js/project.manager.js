var ProjectManager = new (class ProjectManager {
	...
	constructor(actualProject) {
		...
		$('#close, #info a[href="#info"], a[href="#output"]').on("click", () => {
			if (!this.actualProject) {
				window.admob && admob.cacheInterstitial();
	...
	getAll() {
		return Object.entries(localStorage).filter(([name]) => !!localStorage.getItem(name) && name != "-mobilcoder-theme").map(([name, data]) => this.load(name, JSON.parse(data))) || [];
	}
	/**
	 * @param {string} name 

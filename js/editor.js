...
const CodeJar = (editor, highlight) => {
	...
	let at = -1;
	let focus = false;
	/** @type {(code: string) => void} */ let callback;
	let prev; // code content prior keydown event
	let isFirefox = /firefox/i.test(navigator.userAgent);
	editor.attr("contenteditable", isFirefox ? "true" : "plaintext-only");
	editor.attr("spellcheck", "false");
	editor.attr("autocapitalize", "off");
	editor.css("outline", "none");
	editor.css("overflowWrap", "break-word");
	editor.css("overflowY", "auto");
	editor.css("resize", "vertical");
	editor.css("whiteSpace", "pre-wrap");
	editor = editor[0]; highlight(editor);
	...
	function isUndo(event) {
		return isCtrl(event) && event.key === "z";
	}
	function isRedo(event) {
		return isCtrl(event) && event.key === "y";
	}
